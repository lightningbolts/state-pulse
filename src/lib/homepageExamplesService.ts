...
export async function fetchRandomExamplesFromDb(): Promise<HomepageExamples> {
  ...
  if (randomRepresentativeResult.length > 0) {
    ...
    const billsThisYearCount = await legislationCollection.countDocuments({
      $and: [
        sponsorFilter,
        {
          $or: [
...
export const getCachedHomepageExamples = unstable_cache(
