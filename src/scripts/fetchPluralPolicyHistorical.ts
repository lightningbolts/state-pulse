 ...
 */
// Utility to walk directory and collect all JSON file paths
async function collectJsonFiles(dir: string): Promise<string[]> {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      results = results.concat(await collectJsonFiles(fullPath));
  ...
  for (const item of items) {
    ...
    if (executing.length >= limit) {
      ...
      for (let i = executing.length - 1; i >= 0; i--) {
        if (executing[i].then) {
          executing[i].then(() => executing.splice(i, 1), () => executing.splice(i, 1));
        }
      }
    }
  }
...
// Refactored processDirectory logic for a single file
async function processJsonFile(fullPath: string, legislationCollection: any, startAtFile?: string, state?: { skipping: boolean }) {
  // If skipping, check if this is the start file
  if (state && state.skipping) {
    const relPath = path.relative(DATA_DIR, fullPath);
    if (relPath === startAtFile || path.basename(fullPath) === startAtFile) {
  ...
  try {
    ...
    if (Array.isArray(bills) && bills.length > 0) {
      ...
      if (operations.length > 0) {
        ...
        console.log(`Upserted ${operations.length} bills from ${path.basename(fullPath)}`);
      }
...
async function processHistoricalData() {
  ...
  const concurrencyLimit = 6; // Adjust as needed for your system
  await runWithConcurrencyLimit(allFiles, concurrencyLimit, (file) => processJsonFile(file, legislationCollection, startAtFile, state));
