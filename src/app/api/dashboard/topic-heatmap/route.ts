...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!results || results.length === 0) {
      ...
      return jsonWithCdnCache({ success: true, scores: {}, availableTopics: ['all'], selectedTopic, districtType, metadata: { totalDistricts: 0, totalRepresentatives: representatives.length, maxScore: 0, uniqueTopics: 0, processedResults: 0 }}, CACHE.long);
    }
